...
def main():
    prom = "SHITER" + ">"
    install_content = read_install_txt()
    dos = "1"
    os.chdir(tools_folder)
    try:
        while True:
                user_input = input(prom)
                    ...
                    if 'install' in user_input.lower():
                        ...
                        if 'TheFatRat' in user_input:
                            ...
                            if 'dir' in user_input.lower():
                                ...
                                os.system(user_input[8:])
                            elif'cd' in user_input.lower():
                                directory = user_input[11:]
                                if user_input.lower() == 'cd':
                                    prom = "SHITER>"
                                    os.chdir(tools_folder)
                                elif os.path.exists(directory):
                                    os.chdir(directory)
                                    prom = "SHITER:"+directory+">"
                                else:
                                    print(f"Error: Shiter can't find {directory}")
                                    prom = "SHITER>"
                ...
                if user_input.lower() == 'hack_dns-shell':
